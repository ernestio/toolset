...
#extra_options = "#{extra_options} --edition enterprise" if not licensed.to_s.empty?
#`cd $ROOTPATH/ernest && composable set build.path /tmp/composable`
#composable = "composable generate #{env_variables} -exclude='*-aws-connector,*-vcloud-connector,*-azure-connector' -G #{default_version} #{extra_options} definition.yml template.yml --"
#`cd $ROOTPATH/ernest && #{composable}`
#puts "----"
#puts composable
#puts "----"
